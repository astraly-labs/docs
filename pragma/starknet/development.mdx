...
<Warning>
  Every read function below returns a `PragmaPricesResponse`. Validate `price != 0`, `num_sources_aggregated` and the age of `last_updated_timestamp` before using it: the oracle never rejects a stale or empty feed on your behalf. See [Validate the response before using it](./quickstart#validate-the-response-before-using-it).
</Warning>
...
- `last_updated_timestamp`: timestamp of the most recent entry aggregated. The contract does not compare it to the current time: a feed with no active publisher keeps returning its last price. Reject responses older than your own tolerance.
- `num_sources_aggregated`: number of sources in the final answer. `0` (with `price = 0`) means the pair has no usable source anymore; `1` means the median is a single value. Require the minimum your risk profile needs.
