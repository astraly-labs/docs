...
<Warning>
  **SSTRK/USD (market rate) is depreciating.** Its last remaining source, an Ekubo pool with a few dollars of liquidity, no longer qualifies as a price, so the feed is not published anymore. The contract keeps returning the last value it received, so check `last_updated_timestamp` if you still read it, and migrate to `CONVERSION_SSTRK/USD`. The same applies to `UNI/USD` and `LUSD/USD`, which have had no publisher for months.
</Warning>
...
<Tabs>
  <Tab title="Spot">
    ...
    | SSTRK/USD  | 1537084272803954643780   | 8        | ✅      | D    |
