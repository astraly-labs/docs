...
<AccordionGroup>
  ...
  <Accordion icon="ban" title="Depreciating (D)">
    ...
    A discontinued feed does not disappear from the contract: once its last source stops publishing, `get_data_median` keeps returning the last aggregated price with `num_sources_aggregated = 1` and an ever older `last_updated_timestamp`, and once the source is removed it returns zeros. Both cases are caught by the checks described in [Validate the response before using it](./quickstart#validate-the-response-before-using-it).
  </Accordion>
</AccordionGroup>
...
<Warning>
  Whatever the risk level of a feed, always validate `price`, `num_sources_aggregated` and `last_updated_timestamp` before using a response. The oracle never refuses to answer, it only tells you how much to trust the answer. See [Validate the response before using it](./quickstart#validate-the-response-before-using-it).
</Warning>
