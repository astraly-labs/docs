...
The testnet runs Miden node **0.16** (since September 2026, after a chain reset): clients must be on `miden-client 0.16` / `pragma-sdk >= 2.14.4`. Every transaction pays a small fee in the chain's native asset (a 14-entry publish costs ~112 base units); reads through `get_median` are free. Publishers fund their account from the public faucet, the price-pusher does it automatically.

## Asset IDs (faucet_id)

Pragma Miden identifies assets with a `prefix:suffix` format. On the Miden stack the pair is the word `[0, 0, suffix, prefix]` (`push.0.0.{suffix}.{prefix}` in MASM, `prefix` on top):
...
`6:0` to `14:0` are **Miden-only** feeds: Pragma publishes them on Miden but not on Starknet. Decimals follow `pragma-sdk`'s `min(base.decimals, quote.decimals)`, so most pairs use 8 and `USDT/USD` / `XAUT/USD` use 6 — always read the `decimals` of the pair you consume rather than hardcoding it.

The list is driven by `pragma-sdk`'s `STARKNET_PAIR_TO_MIDEN_FAUCET` mapping; new pairs land there first.
